...
<div class="form_body">
	<form action="index.php?module=<?php echo $this -> module;?>&view=<?php echo $this -> view;?>" name="adminForm" method="post">
        ...
        $filter_status = array();
        $filter_status['title'] =  FSText::_('TT Đơn hàng');
        $filter_status['list'] = $this->arr_status;
        $filter_status['field'] = 'name';
        $fitler_config['filter'][] = $filter_status;

        $filter_status = array();
        $filter_status['title'] =  FSText::_('TT Thanh toán');
        $filter_status['list'] = $this->arr_payments;
        $filter_status['field'] = 'name';
        $fitler_config['filter'][] = $filter_status;

        $filter_status = array();
        $filter_status['title'] =  FSText::_('TT Vận chuyển');
        $filter_status['list'] = $this->arr_transport;
        $filter_status['field'] = 'name';
        $fitler_config['filter'][] = $filter_status;
		...
		<div class="form-contents">
			<table border="1" class="tbl_form_contents" bordercolor="#CCCCCC" cellpadding="2">
				<thead>
					...
					<th class="title"  >
                        <?php echo  TemplateHelper::orderTable(FSText :: _('Đơn hàng'), 'status',@$sort_field,@$sort_direct) ; ?>
					</th>
                    <th class="title"  >
                        <?php echo  TemplateHelper::orderTable(FSText :: _('Thanh toán'), 'status_payment',@$sort_field,@$sort_direct) ; ?>
                    </th>
                    <th class="title"  >
                        <?php echo  TemplateHelper::orderTable(FSText :: _('Vận chuyển'), 'status_transport',@$sort_field,@$sort_direct) ; ?>
                    </th>
					...
					<?php if(@$list){?>
						<?php foreach ($list as $row) {
							...
							<tr class="row<?php echo $i%2; ?>">
								...
								<td><?php echo @$this->arr_status[$row->status]->name; ?></td>
                                <td><?php echo @$this->arr_payments[$row ->status_payment]->name; ?></td>
                                <td><?php echo @$this->arr_transport[$row -> status_transport]->name; ?></td>
