 ...
 * @final 21/04/2013
 */ 
class StudentshipModelsStudentship extends FSModels
{
    var $limit;
    var $prefix;
    function __construct()
    {
        ...
        $this->table_category_name = 'fs_studentship_categories';
        $this->table_types = 'fs_studentship_types';
        $this->arr_img_paths = array(
            array('tiny', 500, 500, 'resize_image_fix'),
        );
        $this->table_name = 'fs_studentship';
        $this->img_folder = 'images/studentship/' . date('Y/m/d');
        $this->check_alias = 0;
    ...
    function save()
    {
        ...
        if($_FILES['cover']["name"]) {
            ...
            $path = $this->img_folder.DS.'original'.DS;
            $avatar = $fsFile->uploadImage("cover", PATH_BASE.$path, 2000000, time());
            if($avatar){
                $row['cover'] = $path.$avatar;
                $tinyPath = PATH_BASE.$this->img_folder.DS.'tiny'.DS;
                $fsFile->create_folder($tinyPath);
                $fsFile->resize_image_fix(PATH_BASE.$path.$avatar, $tinyPath.$avatar, 80, 80);
            }
        }
    ...
    */
    ...
    function get_nations(){
        ...
        $query = "SELECT id, name
				  FROM fs_local_countries ORDER BY ordering";
        $sql = $db->query($query);
    ...
    function get_level_education(){
        ...
        $query = "SELECT id, title
				  FROM fs_studentship_level_education ORDER BY ordering";
        $sql = $db->query($query);
        return $db->getObjectList();
    }

    function get_scholarship_value(){
        global $db;
        $query = "SELECT id, title
				  FROM fs_studentship_scholarship_value ORDER BY ordering";
        $sql = $db->query($query);
        return $db->getObjectList();
    }
