...
class Upload{
	 ...
	 */
	function uploadImage($input_tag_name, $path ,$size_max= 4000000, $suffix = '',$crop = 0,$crop_width = 0, $crop_height = 0)
	{
...
//	 	chmod($img_destination, 0655);
	...
	*/
	...
	function resize_image_fix($img_source, $img_destination, $crop_width = 1, $crop_height = 1){
		...
		// writeFile(PATH_BASE.'admin/log.log', $file_ext);
		if($file_ext == "png"){
			$newpic = imagecreatetruecolor($crop_width,$crop_height)
			or die("Cannot Initialize new GD image stream when cropped");
			$alpha_channel = imagecolorallocatealpha($newpic, 0, 0, 0, 127);
			imagecolortransparent($newpic, $alpha_channel);
			// Fill image
			imagefill($newpic, 0, 0, $alpha_channel);
			// Copy from other
			if(!imagecopyresampled($newpic, $source,0,0, $x_crop, $y_crop, $real_width, $real_height, $old_width, $old_height)){
				echo "Not copy and resize part of an image with resampling when cropped";
				return false;
			...
			imagesavealpha($newpic,true);
			// Save PNG
			$result  = imagepng($newpic, $img_destination, 9);
