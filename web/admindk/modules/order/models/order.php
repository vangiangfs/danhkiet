	...
	class OrderModelsOrder extends FSModels
	{
		...
		function setQuery(){
            ...
            if(isset($_SESSION[$this -> prefix.'filter1'])){
                ...
                if($filter){
                    $filter = intval($filter);
                    $where .= ' AND a.status_payment =  "'.$filter.'" ';
                }
            }
            ...
            if(isset($_SESSION[$this -> prefix.'filter2'])){
                ...
                if($filter){
                    $filter = intval($filter);
                    $where .= ' AND a.status_transport =  "'.$filter.'" ';
                }
            }
		...
		function get_data_order(){
			...
			global $db;
			$query = "  SELECT a.*, b.name as product_name, b.alias as product_alias, b.category_alias, b.colors as product_colors, b.size as product_size, b.material as product_material
    					FROM fs_order_items AS a
    					INNER JOIN fs_products_fashion AS b on a.product_id = b.record_id
    					WHERE a.order_id = $id";
			$db->query($query);
			$result = $db->getObjectList();

            $sizes = $this->get_records('', 'fs_extends_size', 'id, name', '', '', 'id');
            $materials = $this->get_records('', 'fs_extends_material', 'id, name', '', '', 'id');
            $i = 0;
            foreach($result as $item){
                $result[$i]->ordSizes = array();
                $arrSizes = explode(',', $result[$i]->product_size);
                foreach($arrSizes as $key)
                    if(isset($sizes[$key])) $result[$i]->ordSizes[$key] = $sizes[$key];

                $result[$i]->ordMaterials = array();
                $arrMaterials = explode(',', $result[$i]->product_material);
                foreach($arrMaterials as $key)
                    if(isset($materials[$key])) $result[$i]->ordMaterials[$key] = $materials[$key];

                $result[$i]->ordColors = unserialize($result[$i]->product_colors);
